...
*The presence of ~7,000 orphaned records across the dataset initially suggests potential data quality issues. However, after investigating the temporal distribution and user patterns, I have categorized these into two distinct phenomena:
    ...
    * Ad-Hoc/Event-Based Deployment: The raw data contains one `'Springfest 2022'` end-station label (see **Step 05** below) — a pop-up hub never registered in `station_info`. Most orphaned legs, however, are unregistered catalog IDs (e.g., `7125`, `7188`), not one-off events.
...
* result: **1 trip** in raw rentals with the end station named **'Springfest 2022'** (`rental_id` 26160487).
* **Trip profile:** March 14, 2022 · Classic bike · Pay-as-you-ride · Start: `3797` (21st/University) · End name: `Springfest 2022` · **`end_station_id` = NULL** · Duration: 12 minutes.
* **Discussion:** This is almost certainly a temporary pop-up dock deployed for a campus event, logged by name but never assigned a catalog `station_id`. A single 12-minute trip is far too small to represent festival-scale demand — it is a metadata edge case, not evidence of an event-driven utilization spike.
* **Data Provenance:** Because `end_station_id` is NULL, the record cannot join to `station_info` and behaves like other orphaned legs documented in Step 03.
* **Pipeline Decision (aligned with production CTE):**
   * The trip is **documented here in the raw audit** as proof that the telemetry system logs ad-hoc hubs.
   * It is **excluded from the 16,504-row production dataset** by the `CleanedRentals` filter (`end_station_id IS NOT NULL`), which requires complete start/end station keys for dock-capacity and flow analysis.
   * This is intentional: incomplete station metadata trips are preserved in raw logs for data-governance review but omitted from forecasting and bottleneck models where spatial joins are mandatory.
   * **Downstream implication:** Section 03 classic-bike analysis runs entirely on the production CTE — Springfest cannot influence bike-type or event-spike conclusions in that dataset.
